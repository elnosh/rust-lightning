...
pub struct UnsignedNodeAnnouncementV2 {
	...
	/// IPv4 addresses on which this node is reachable, with their ports
	pub ipv4_addrs: Option<Vec<(u32, u16)>>,
	/// IPv6 addresses on which this node is reachable, with their ports
	pub ipv6_addrs: Option<Vec<([u8; 16], u16)>>,
	/// Tor v3 onion addresses on which this node is reachable, with their ports
	pub tor_v3_addrs: Option<Vec<([u8; 35], u16)>>,
	/// DNS hostnames on which this node is reachable, with their ports
	pub dns_hostnames: Option<Vec<(Hostname, u16)>>,
	/// Excess data which was signed as a part of the message which we do not (yet) understand how
	/// to decode.
	pub signed_excess: Vec<u8>,
}
...
pub struct NodeAnnouncementV2 {
	/// A BIP-340 Schnorr signature by the node key
...
pub struct ChannelAnnouncementV2 {
	/// A BIP-340 Schnorr signature over a MuSig2-aggregated key
	pub signature: schnorr::Signature,
	/// The actual announcement
...
pub struct ChannelUpdateV2 {
	/// A BIP-340 Schnorr signature of the channel update
	pub signature: schnorr::Signature,
	/// The actual channel update
...
pub trait RoutingMessageHandler: BaseMessageHandler {
	...
	/// Handle an incoming `node_announcement_2` message, returning `true` if it should be forwarded
	/// on, `false` or returning an `Err` otherwise.
	///
	/// If `their_node_id` is `None`, the message was generated by our own local node.
	fn handle_node_announcement_v2(
		&self, their_node_id: Option<PublicKey>, msg: &NodeAnnouncementV2,
	) -> Result<bool, LightningError>;
	/// Handle a `channel_announcement_2` message, returning `true` if it should be forwarded on,
	/// `false` or returning an `Err` otherwise.
	///
	/// If `their_node_id` is `None`, the message was generated by our own local node.
	fn handle_channel_announcement_v2(
		&self, their_node_id: Option<PublicKey>, msg: &ChannelAnnouncementV2,
	) -> Result<bool, LightningError>;
	/// Handle an incoming `channel_update_2` message, returning the node IDs of the channel
	/// participants if the message should be forwarded on, `None` or returning an `Err` otherwise.
	///
	/// If `their_node_id` is `None`, the message was generated by our own local node.
	fn handle_channel_update_v2(
		&self, their_node_id: Option<PublicKey>, msg: &ChannelUpdateV2,
	) -> Result<Option<(NodeId, NodeId)>, LightningError>;
	...
	/// Gets `channel_announcement_2` and `channel_update_2` messages required to dump our routing
	/// table to a remote node, starting at the `short_channel_id` indicated by `starting_point` and
	/// including announcements for a single channel.
	...
	/// Gets a `node_announcement_2` message required to dump our routing table to a remote node,
	/// starting at the node *after* the provided pubkey and including up to one announcement
...
impl<T: RoutingMessageHandler + ?Sized, R: Deref<Target = T>> RoutingMessageHandler for R {
	...
	fn handle_node_announcement_v2(
		...
		self.deref().handle_node_announcement_v2(their_node_id, msg)
	}
	fn handle_channel_announcement_v2(
		&self, their_node_id: Option<PublicKey>, msg: &ChannelAnnouncementV2,
	) -> Result<bool, LightningError> {
		self.deref().handle_channel_announcement_v2(their_node_id, msg)
	}
	fn handle_channel_update_v2(
		&self, their_node_id: Option<PublicKey>, msg: &ChannelUpdateV2,
	) -> Result<Option<(NodeId, NodeId)>, LightningError> {
		self.deref().handle_channel_update_v2(their_node_id, msg)
	}
	fn get_next_channel_announcement(
		&self, starting_point: u64,
	) -> Option<(ChannelAnnouncement, Option<ChannelUpdate>, Option<ChannelUpdate>)> {
	...
	fn get_next_channel_announcement_v2(
		...
		self.deref().get_next_channel_announcement_v2(starting_point)
	}
	fn get_next_node_announcement(
		&self, starting_point: Option<&NodeId>,
	) -> Option<NodeAnnouncement> {
	...
	fn get_next_node_announcement_v2(
		...
		self.deref().get_next_node_announcement_v2(starting_point)
	}
	fn handle_reply_channel_range(
		&self, their_node_id: PublicKey, msg: ReplyChannelRange,
	) -> Result<(), LightningError> {
