	...
	for DefaultResourceManager
{
	fn read<R: Read>(
		...
		config.validate().map_err(|_| DecodeError::InvalidValue)?;
		_init_and_read_len_prefixed_tlv_fields!(reader, {
...
impl AggregatedWindowAverage {
	...
	fn value_at_timestamp(&mut self, timestamp_unix_secs: u64) -> i64 {
		...
		(self.aggregated_revenue_decaying.value_at_timestamp(timestamp) as f64 / divisor).round()
			as i64
	}
}
...
mod tests {
	...
	fn test_slots_from_salt() {
		...
		let entropy_source = TestKeysInterface::new(&[0; 32], Network::Testnet);
		let general_bucket = GeneralBucket::new(0, 100, 100_000_000).unwrap();
		let scid = 21;
	...
	fn test_aggregated_window_average() {
		...
		let average_duration = Duration::from_secs(avg_weeks as u64 * week_secs);
		...
		for _ in 0..num_points {
			let mut buf = [0u8; 8];
			prng.process_in_place(&mut buf);
			let ts_offset = u64::from_le_bytes(buf) % (duration_weeks * week_secs);
		...
		for w in 1..=duration_weeks {
			...
			while data_idx < num_points && sorted_ts[data_idx] <= sample_time {
				avg.add_value(sorted_vals[data_idx], sorted_ts[data_idx]);
				data_idx += 1;
			}
