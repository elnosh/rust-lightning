...
use core::{fmt::Display, ops::Deref, time::Duration};
	...
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			ForwardingOutcome::Forward(signal) => {
				write!(
					f,
					"Forward with accountable signal {}",
					accountable_into_bool(*signal).unwrap_or(false)
				)
			},
		...
		match self.slots_for_amount(outgoing_scid, htlc_amount)? {
			Some(slots) => match self.channels_slots.entry(outgoing_scid) {
				...
				Entry::Occupied(mut entry) => {
					// TODO: add asserts here to check that slots were previously
					// unoccupied (?)
...
impl<ES: Deref> Writeable for Channel<ES>
where
	ES::Target: EntropySource,
{
...
struct ChannelReadArgs<ES: Deref>
where
	ES::Target: EntropySource,
{
	entropy_source: Arc<ES>,
...
impl<ES: Deref> ReadableArgs<ChannelReadArgs<ES>> for Channel<ES>
where
	ES::Target: EntropySource,
{
	fn read<R: Read>(
		...
		for _ in 0..channels_assigned_count {
			...
			println!(
				"outgoing_scid {}. scid {}. general_bucket.scid {}",
				outgoing_scid, scid, general_bucket.scid
			);
			if outgoing_scid != general_bucket.scid {
		...
		let mut congestion_bucket = BucketResources::new(
			congestion_bucket_slots_allocated,
		...
		let mut protected_bucket = BucketResources::new(
			protected_bucket_slots_allocated,
		...
		for (_, pending_htlc) in pending_htlcs.iter() {
			match pending_htlc.bucket {
				...
				BucketAssigned::Congestion => {
					congestion_bucket
						.add_htlc(pending_htlc.incoming_amount_msat)
						.map_err(|_| DecodeError::InvalidValue)?;
				},
...
pub struct DefaultResourceManager<ES: Deref>
where
	ES::Target: EntropySource,
{
...
impl<ES: Deref> DefaultResourceManager<ES>
where
	ES::Target: EntropySource,
{
	...
	pub fn add_htlc(
		...
		let (accountable_signal, bucket_assigned) = if !incoming_accountable {
			if self.general_available(
				...
				(accountable_from_bool(false), BucketAssigned::General)
			...
			if self.sufficient_reputation(
				...
				if incoming_channel.protected_bucket.resources_available(incoming_amount_msat) {
					...
					.can_add_htlc(outgoing_channel_id, incoming_amount_msat)?
				{
					(accountable_from_bool(true), BucketAssigned::General)
				} else {
					return Ok(ForwardingOutcome::Fail);
				}
			} else {
		...
		println!("HTLC GOT ADDED TO {:?} BUCKET", bucket_assigned);
...
impl<ES: Deref> Writeable for DefaultResourceManager<ES>
where
	ES::Target: EntropySource,
{
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
...
impl<ES: Deref> ReadableArgs<ES> for DefaultResourceManager<ES>
where
	ES::Target: EntropySource,
{
	fn read<R: Read>(
...
mod tests {
	...
	fn test_general_bucket_add_htlc_over_max_liquidity() {
		...
		assert!(general_bucket.channels_slots.get(&scid).is_some());
	...
	fn test_general_bucket_add_htlc() {
		...
		let channel_slots = general_bucket.channels_slots.get(&scid).unwrap();
		for slot in slots_occupied.iter() {
			assert_eq!(
				channel_slots.0.iter().find(|slot| slot.0 == slot_occupied).unwrap().1,
				true
			);
			assert_eq!(general_bucket.slots_occupied[*slot as usize], true);
		}
	...
	#[test]
	fn test_bucket_resources_add_htlc_over_slots_available() {
		let mut bucket_resources = test_bucket_resources();
		...
		// Test that it errors if it tries to add one more HTLC to go over the slot count
		// limit
		assert!(bucket_resources.add_htlc(10).is_err());
	}

	#[test]
	fn test_bucket_resources_add_htlc_over_liquidity_available() {
		// Test that it errors if it tries to add HTLC that will go over the liquidity
		// available.
		let mut bucket_resources = test_bucket_resources();
		let liquidity_available = bucket_resources.liquidity_allocated;
		assert!(bucket_resources.add_htlc(liquidity_available - 1000).is_ok());
		assert!(bucket_resources.add_htlc(2000).is_err());
	}
	...
	#[test]
	fn test_sufficient_reputation_success() {
		let rm = create_test_resource_manager_with_channels();
		add_reputation(&rm, OUTGOING_SCID, 100_000);
		assert_eq!(test_sufficient_reputation(&rm), true);
	}

	#[test]
	fn test_sufficient_reputation_high_in_flight_risk() {
		let rm = create_test_resource_manager_with_channels();
	...
	#[test]
	fn test_sufficient_reputation_high_incoming_revenue_threshold() {
		let rm = create_test_resource_manager_with_channels();
		add_reputation(&rm, OUTGOING_SCID, 10_000);

		// Build high revenue on incoming channel to raise the threshold
		let current_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
		let mut channels = rm.channels.lock().unwrap();
		let incoming_channel = channels.get_mut(&INCOMING_SCID).unwrap();
		// Add high revenue value
		incoming_channel.incoming_revenue.add_value(50_000, current_time).unwrap();
		drop(channels);

		// Outgoing reputation should not meet the high incoming revenue threshold
		assert_eq!(test_sufficient_reputation(&rm), false);
	}
	...
	fn test_sufficient_reputation_exactly_at_threshold() {
		...
		// Calculate the exact in-flight risk for our test HTLC
		let in_flight_risk = rm.htlc_in_flight_risk(FEE_AMOUNT, CLTV_EXPIRY, CURRENT_HEIGHT);

		// Set up specific reputation to test boundary condition
		let current_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
		let mut channels = rm.channels.lock().unwrap();
		...
		// Set outgoing reputation to exactly match threshold plus in-flight risk
		...
		// With reputation exactly at threshold (after subtracting in-flight risk), should return true
		assert_eq!(test_sufficient_reputation(&rm), true);
	}
	...
	fn create_test_resource_manager_with_channels() -> DefaultResourceManager<Arc<TestKeysInterface>>
	{
		...
		let resource_manager = DefaultResourceManager::new(config, entropy_source);
		let channel_type = ChannelTypeFeatures::anchors_zero_fee_commitments();
		resource_manager.add_channel(&channel_type, INCOMING_SCID, 1_000_000_000, 114).unwrap();
		resource_manager.add_channel(&channel_type, OUTGOING_SCID, 1_000_000_000, 114).unwrap();
		resource_manager
	}
	...
	// TODO: test
	// resolving htlcs
	// - effect on reputation/revenue
	// - misuses congestion called appropriately
	//
	// Decaying average
	// Integration - style tests with multiple nodes and building reputation

	// On appropriate structs, re-construct them from readargs
	// What to test for ResourceManager:
	// - detecting correct reputation.
	...
	fn test_add_htlc_unaccountable_general_available() {
		...
		assert_eq!(result.unwrap(), ForwardingOutcome::Forward(accountable_from_bool(false)));
		assert_eq!(
			get_htlc_bucket(&rm, INCOMING_SCID, htlc_id, OUTGOING_SCID).unwrap(),
			BucketAssigned::General
		);
	...
	fn test_add_htlc_unaccountable_protected_sufficient_reputation() {
		...
		let result = add_test_htlc(&rm, false, htlc_id);
		assert!(result.is_ok());
		assert_eq!(result.unwrap(), ForwardingOutcome::Forward(accountable_from_bool(true)));
		assert_eq!(
			get_htlc_bucket(&rm, INCOMING_SCID, htlc_id, OUTGOING_SCID).unwrap(),
			BucketAssigned::Protected
		);
	...
	fn test_add_htlc_unaccountable_congestion_already_has_htlc() {
		...
		assert_eq!(result_1.unwrap(), ForwardingOutcome::Forward(accountable_from_bool(true)));
		assert_eq!(
			get_htlc_bucket(&rm, INCOMING_SCID, htlc_id, OUTGOING_SCID).unwrap(),
			BucketAssigned::Congestion
		);
	...
	fn test_add_htlc_unaccountable_insufficient_reputation_fails() {
		// If outgoing channel does not have sufficient reputation and general and congestion
		// buckets are full, HTLC forward fails.
		let rm = create_test_resource_manager_with_channels();
		fill_general_bucket(&rm, INCOMING_SCID);
		mark_congestion_misused(&rm, INCOMING_SCID, OUTGOING_SCID);
	...
	#[test]
	fn test_add_htlc_unaccountable_sufficient_reputation_but_protected_full() {
		// Test HTLC forward fails if sufficient reputation but all buckets are full
		let rm = create_test_resource_manager_with_channels();
