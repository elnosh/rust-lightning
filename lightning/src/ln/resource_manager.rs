...
use crate::{
	...
	sign::EntropySource,
	sync::Mutex,
	util::ser::{ReadableArgs, Writeable, Writer},
};
...
impl<'a, ES: Deref> ReadableArgs<ES> for Channel<'a, ES>
where
	ES::Target: EntropySource,
{
	fn read<R: Read>(reader: &mut R, entropy_source: ES) -> Result<Channel<'a, ES>, DecodeError> {
		unimplemented!()
	}
}

struct DefaultResourceManager<'a, ES: Deref>
where
	ES::Target: EntropySource,
{
...
where
	ES::Target: EntropySource,
{
	fn new(config: ResourceManagerConfig, entropy_source: ES) -> Self {
...
impl<'a, ES: Deref> ReadableArgs<ES> for DefaultResourceManager<'a, ES>
where
	ES::Target: EntropySource,
{
	fn read<R: Read>(
